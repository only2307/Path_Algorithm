...
def visualize_maze(matrix, bonus, start, end, route=None, visited=None):
    ...
    if visited:
        ...
        plt.scatter([i[1] for i in visited], [-i[0] for i in visited],
                    marker='.', s=35, color='pink')

    if route:
        for i in range(len(route)-2):
            plt.scatter(route[i+1][1],-route[i+1][0],
                        marker=direction[i],color='darkgreen')
    ...
    print(f'Length of the path: {len(route)}')
    print(f'Total cost: {len(visited)}')
